...
const TradePanel: React.FC = () => {
  ...
  const { availableBalances, placeOrder, openFuturesPosition, isPaperTrading } = useTradeHistory();
  ...
  const buttonDisabled = !parseFloat(amount) || !parseFloat(price) || isSubmitting;
  
  const getButtonText = () => {
    if(isSubmitting) return "Processing...";
    return isBuy ? `Long ${baseAsset}` : `Short ${baseAsset}`;
  }
    ...
    <div>
      ...
      <div className="relative">
        <input id={id} type="number" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
          className="w-full bg-brand-bg border border-brand-border rounded-md pl-3 pr-14 py-2 text-brand-text-primary placeholder-brand-secondary text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary font-mono" />
  ...
  return (
    <div className="bg-brand-surface border border-brand-border rounded-lg p-4 h-full flex flex-col">
       <div className="flex justify-center border-b border-brand-border mb-4">
        <h3 className="text-brand-text-primary font-semibold text-base py-2">Trade Perpetuals</h3>
      </div>
      ...
      <div className="flex border-b border-brand-border mb-4">
        <button
          ...
          className={`w-1/2 py-2 text-center font-semibold transition-colors ${
            isBuy ? 'text-brand-green border-b-2 border-brand-green' : 'text-brand-secondary hover:text-brand-text-primary'
          }`}
        >Long</button>
        <button
          onClick={() => { setActiveTab(OrderType.SELL); setError(null); }}
          className={`w-1/2 py-2 text-center font-semibold transition-colors ${
            !isBuy ? 'text-brand-red border-b-2 border-brand-red' : 'text-brand-secondary hover:text-brand-text-primary'
          }`}
        >Short</button>
      </div>
      ...
      <div className="space-y-4 flex-grow">
        ...
        <div className="flex justify-between">
            {[25, 50, 75, 100].map(p => (
                <button key={p} onClick={() => handlePercentage(p)}
                    className="text-xs bg-brand-bg border border-brand-border rounded-md px-3 py-1 hover:bg-brand-border transition-colors w-[23%] text-brand-text-secondary hover:text-brand-text-primary">
                    {p}%
                </button>
            ))}
        </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="text-xs text-brand-secondary mb-1 block">Leverage: <span className="text-brand-text-primary font-mono">{leverage}x</span></label>
        ...
        <div className="border-t border-brand-border pt-4 space-y-2 text-xs">
          <div className="flex justify-between text-brand-secondary">
            <span>Available</span>
            <span className="font-mono text-brand-text-primary">{availableBalances.usdt.toFixed(2)} {quoteAsset}</span>
          </div>
           <div className="flex justify-between text-brand-secondary">
            <span>Margin</span>
            <span className="font-mono text-brand-text-primary">{margin} {quoteAsset}</span>
          </div>
          <div className="flex justify-between text-brand-secondary">
            <span>Liq. Price (est.)</span>
            <span className="font-mono text-brand-text-primary">{liquidationPrice} {quoteAsset}</span>
          </div>
        </div>
      </div>
